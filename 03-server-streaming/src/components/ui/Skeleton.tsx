...
// and delta (mt-1 text-xs, 16px line-height) lines exactly, so a KPI tile's
// height doesn't change when its Suspense boundary resolves.
export function KpiValueSkeleton({ withDelta = false }: { withDelta?: boolean }) {
  return (
    <>
...
// as `rows` makes the fallback match the resolved table's height exactly.
