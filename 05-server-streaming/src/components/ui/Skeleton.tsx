...
// classes, via the shared DividedList) so the Suspense fallback occupies the
...
// into (see RevenueChartView/ExpenseBreakdownView), so the fallback reserves
...
// Bar heights/margins mirror the real value (mt-2 text-2xl, 32px line-height)
// and delta (mt-1 text-xs, 16px line-height) lines exactly. Always shows
// both, regardless of whether that particular tile ends up with a delta -
// picking that per-tile here would mean hardcoding, by position, which
// ones have one (brittle: breaks silently if a tile is reordered or
// added). The KPI grid stretches every cell to the row's tallest sibling
// anyway, so this costs nothing for the tiles that don't.
export function KpiValueSkeleton() {
  return (
    <>
      <Bar className="mt-2 h-8 w-24" />
...
// bars do too - matching the real 48px total exactly
...
// genuinely variable-length data, not something a fallback can fully hide.
